...
import inlineStyles from './cmtitemStyle'

// 导入评论项的样式文件【这种直接 import 路径 的 CSS 导入模式，并不是模块化的CSS 】
// import '../../css/commentItem.css'
// 默认情况下，如果没有为 CSS 启用模块化，则接收到的 itemStyles 是一个空对象，因为 .css 样式表中，
// 不能直接通过 JS 的 export default 导出对象

// 当启用 CSS 模块化之后，导入样式表得到的 itemStyles 就变成一个样式对象，其中，属性名是样式表中定义的类名
// 属性值是自动生成的一个复杂的类名（防止类名通用）
import itemStyles from '../../css/commentItem.css'
console.log(itemStyles)


// 封装一个评论项组件，此组件由于不需要自己的私有数据，所以直接定义为一个无状态组件
// 将评论项抽离成一个单独的组件，
export default function CommentItem(props) {
    // 注意：如果要使用style属性，为JSX 语法创建的DOM元素设置样式，不能像网页中那么写；
    // 而是要使用JS语法来写样式
    // 在写 style 样式的时候，外层的 {} 表示要写JS代码，内层的{}表示用一个JS对象表示样式
    // 注意：JSX 中，style的样式规则中，如果属性值的单位是px，则px可以省略，直接写一个数值即可
